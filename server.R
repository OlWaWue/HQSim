...
shinyServer(function(input, output, session) {
  ...
  observeEvent(input$loadPT,{
    ...
    tryCatch({
      ...
      updateCheckboxInput(session, inputId = "has_MTX", value=pat_info$MTX[1])
  ...
  get_cov_from_dataset <- function(dataset, times) {
    ...
    MTX <- dataset[dataset$evid == 2 & dataset$mtx !=".",]
    ...
    if(nrow(MTX)==0){
      pat_MTX = data.frame(time=c(0),
                            value=c(as.numeric(input$has_MTX)))
    } else {
      pat_MTX = data.frame(time=c(0,MTX$time),
                            value=c(as.numeric(input$has_MTX), as.numeric(as.character(MTX$mtx))))
    }
    ...
    for(i in 1:nrow(pat_MTX)){
      time_dep_cov$MTX <- ifelse(pat_MTX$time[i] <= time_dep_cov$time, pat_MTX$value[i], time_dep_cov$MTX)
    }
  ...
  resetApp <- function(){
    ...
    updateCheckboxInput(session, inputId = "has_MTX", value=F)
  ...
  updatePKPlot <- function(){
    ...
    app_data$params <- c(input$WT, input$CRCL, input$has_MTX)
    ...
    if(app_data$tdm_samples_available) {
        ...
        ind_plot <- ggplot(ind_plot_data)  +
          ...
          geom_point(data=tdm_data, aes(x=time, y=conc, colour="tdm"), size=3, shape=1, stroke=2) + plot_theme + xlab("") + ylab("Hydroxychloroquine Plasma Concentration [µg/L]") +
          ...
          scale_colour_manual(values=c("ind"="blue",
                                       ...
                                       "pop"="black"),
                                        ...
                                        labels=c("Individual Prediction", 
                                                 "Population Prediction","TDM Data")
                              ) + 
    ...
    pop_boxplot <- ggplot(data=data.frame(conc=dat_mc[,ncol(dat_mc)], time="")) + geom_boxplot(aes(x=time, y=conc)) + plot_theme +
      theme(axis.text.y = element_blank(), axis.title.y = element_blank(), 
            axis.ticks.y = element_blank())+ ggtitle("C last [µg/L]" , "Population") + 
    ...
    pop_plot <- ggplot(data=plot_dat)  + geom_line(aes(x=TIME, y=CP, colour="pop")) +
      ...
      plot_theme + xlab("") + ylab("Hydroxychloroquine Plasma Concentration [µg/L]") + ggtitle("Population Prediction Using Patient Covariates", "Select area and double click for zoom and unzoom") +
      ...
      scale_colour_manual(values=c("pop"="blue"),
                          guide = guide_legend(override.aes = list(
                            linetype =  c(1),
                            shape = c(NA),
                            fill = c("white")
                          ...
                          title=""),
                          labels=c("Population Prediction")
      ) + 
    ...
    if(app_data$tdm_samples_available) {
        ...
        dist_plots[[1]] <- ggplot() +theme_bw()+ 
          geom_density(data=pop_pars, aes(x=ka, y=..density.., fill="pop"), colour="blue", size=.5,alpha=0.25, linetype=1) +
          geom_density(data=ind_pars, aes(x=ka, y=..density.., fill="ind"), colour="red", size=.5,alpha=0.25, linetype=1) +
        ...
        dist_plots[[2]] <- ggplot(  ) +theme_bw()+ 
          geom_density(data=pop_pars, aes(x=V, y=..density.., fill="pop"), colour="blue", size=.5,alpha=0.25, linetype=1) +
          geom_density(data=ind_pars,aes(x=V, y=..density.., fill="ind"), colour="red", size=.5,alpha=0.25, linetype=1)+
  ...
  output$data_set <- DT::renderDataTable({
    ...
    if(!app_data$disc_shown){
      ...
      showModal(modalDialog(
        title = "Disclaimer",
        HTML(paste("<B>daGama - HQSim has been created for personal use only! </B><BR>", 
                   "There is no guarantee for the correctness of results obtained via HQSim Using data generated by the application is at the risk of the daGama - HQSim user alone!", 
  ...
  updateAdapted_pk_plot <- function(){
    ...
    if(app_data$forecast_advanced){
      ...
      new_dosing_events <- data.frame(time = time, amt = amt)
    }
    ...
    ## Build raw individual PK plot
    p <- ggplot(pk_data) + 
      annotate("text", x=middle_of_the_plot, y=y_max, label=paste("AUC 24h: ", round(AUC_24[1],1) , " (95% PI: ", round(AUC_24[2],1), "-", round(AUC_24[3],1), ") µg x h /L", sep="" ), size=6 ) +
      geom_ribbon(aes(ymin=s1, ymax=s2, x=time, fill="s1" ), alpha=0.15) + 
      geom_ribbon(aes(ymin=s3, ymax=s4, x=time, fill="s2" ), alpha=0.15) + 
      geom_ribbon(aes(ymin=s5, ymax=s6, x=time, fill="s3" ), alpha=0.15) + 
      geom_ribbon(aes(ymin=s7, ymax=s8, x=time, fill="s4" ), alpha=0.15) + 
      geom_line(aes(y=max, x=time, colour="pred"))+
      plot_theme + xlab("") + ylab("Hydroxychloroquine Plasma Concentration [µg/L]") + ggtitle("Prediction of new Dosing Scheme beginning at last event (Dose or TDM)", "Select area and double click for zoom and unzoom") + 
      geom_ribbon(aes(ymin=input$low.target, ymax=input$high.target, x=time,fill="target"), alpha=0.3) +  scale_x_datetime(labels = date_format("%a %d.%m.%Y\n%H:%M", tz = "CET")) +
    ...
    if (app_data$tdm_samples_available){
      ...
      p <- p+ scale_colour_manual(values=c("pred"="red"),
                                  guide = guide_legend(override.aes = list(
                                    linetype =  c(1),
                                    shape = c(NA),
                                    fill = c("white")
  ...
  output$modelfile <- renderText({
    ...
    ## Show the correct JAGS model file
      ...
      includeText("Carmichael_et_al.bug")
  ...
  output$info <- renderText({
    ...
    paste("<h4>1. Please enter below:</h4><h5><BR>Patient covariates, dosing and if available TDM data. Then, click the <B>\"Analyze Data\"</B> Button.</h5><BR>!ONLY MEANINGFUL COVARIATE IS MTX COMEDICATION!")
  ...
  output$info.pk <- renderText({
    ...
    paste("<h4>2. Time-Plasmaconcentration-Curves:</h4><h5><BR>Click the <B>\"Automatic Dose Adaptation\"</B> Button for automatic dosing proposal or choose <B>\"Manual Dose Adaptation\"</B>.</h5><BR>THERE IS NO VALIDATED TARGET CONCENTRATION!")
  ...
  output$info.model <- renderText({
    ...
    paste("<h4>Model File</h4><h5><BR>This is a JAGS compatible model file of the popPK Model <b>Carmichael et al. Ther Drug Monit (2003) 25(6):671–681</B></h5>")
  ...
  output$info.about <- renderText({
    ...
    paste("<h4>About</h4><h5><BR>daGama - HQSim is free and open source: at <a href=\"https://github.com/OlWaWue/HQSim\">GitHub</a></h5>")
  ...
  observeEvent(input$but.adapt, {
    ...
    obj_fun_all <- function(par, data_set, N) {
      ...
      temp <- data.frame(time=time, amt=amt)
    ...
    obj_fun_dose <- function(par, II, data_set, N) {
    ...
    obj_fun_ii <- function(par, AMT, data_set, N) {
    ...
    if(input$adapt.what==4) {
      ...
      opt_res <- optim(par=c(AMT=1000, II=12), fn=obj_fun_all, data_set=app_data$data_set, N=input$adapt.n)
      ...
      opt_res <- optim(par=c(AMT=input$adapt.dose), fn=obj_fun_dose, data_set=app_data$data_set, N=input$adapt.n, II=input$adapt.ii, method="Brent", lower=0,upper=5000,
                       control=list(maxit=500))
      
      updateNumericInput(session, inputId = "adapt.dose", value = round(as.numeric(as.character(opt_res$par[1])),2) )
    } else if(input$adapt.what==2) {
      opt_res <- optim(par=c(II=input$adapt.ii), fn=obj_fun_ii, data_set=app_data$data_set, N=input$adapt.n, AMT=input$adapt.dose, method="Brent", lower=6,upper=24,
                       control=list(maxit=500))
  ...
  observeEvent(input$REM_FORE_OK,{
    ...
    if(nrow(app_data$user_forecast_data_set)>=1){
      ...
      colnames(fore_data_set) <- c('time', 'amt')
  ...
  observeEvent(input$ADD_OK,{
    ...
    if(nrow(app_data$user_data_set)>0){  
      ...
      app_data$user_data_set$MTX <- as.character(app_data$user_data_set$MTX )
