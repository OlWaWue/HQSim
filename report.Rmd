...
title: "TDM Report - Hydroxychloroquine"
output: 
  pdf_document:
    latexengine: xelatex
df_print: paged
...
Receives MTX: **`r if(input$has_MTX) {"Yes"}else{"No"}`**
...
colnames(tmp) <- c('Date', 'Time', 'Dose [mg]')
...
colnames(tmp) <- c('Interdose Interval [h]', 'Dose [mg]')              
kable(tmp, format = "latex")  %>%
    kable_styling(position = "left")
} ` 
